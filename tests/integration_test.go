...
func TestIntegration_PGMFormat(t *testing.T) {
	if binaryPath == "" {
		t.Skip("binary not found")
	}
	...
	outPath := filepath.Join(outDir, "kit.pgm")
	cmd := exec.Command(binaryPath, wavPath, "-f", "pgm", "-o", outPath)
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("command failed: %v\noutput: %s", err, string(out))
	}
	...
	if _, err := os.Stat(outPath); os.IsNotExist(err) {
		t.Fatalf("expected PGM output file %s does not exist", outPath)
	}
}
