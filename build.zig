...
pub fn build(b: *std.Build) void {
    ...
    const go_build_step = if (link_go and go_archive.len == 0) blk: {
        const ldflags = std.mem.concat(b.allocator, u8, &.{ "-X ", "github", ".", "com/g-lok/chirashi/cmd.version=", version }) catch @panic("OOM");
        ...
        const gb = b.addSystemCommand(&.{
            ...
            "netgo",                          "-ldflags",
            ldflags,                          "-o",
            "internal/engine/go_engine.a", "main.go",
        });
    ...
    const zig_source = if (os_is_linux)
        b.path("internal/engine/extractor_stub.zig")
    else
        b.path("internal/engine/extractor.zig");
    ...
    if (!os_is_linux) {
        // Manual Zig bindings for REX SDK C API (avoids zig translate-c which
        // hangs on REX.h due to packed structs/macros in Zig 0.16.0).
        const rex_module = b.createModule(.{
            .root_source_file = b.path("internal/engine/rex_bindings.zig"),
            .target = target,
            .optimize = optimize,
            .link_libc = true,
        });
        root_module.addImport("rex_c", rex_module);
    }
    ...
    if (target.result.os.tag == .windows) {
        exe.root_module.addCSourceFile(.{ .file = b.path("internal/engine/rex/REX.c"), .flags = &.{ "-DREX_WINDOWS=1", "-DREX_MAC=0" } });
        exe.root_module.linkSystemLibrary("version", .{});
    } else if (target.result.os.tag == .macos) {
        exe.root_module.addFrameworkPath(b.path("internal/engine/libs/macos"));
    ...
    exe.root_module.addIncludePath(b.path("internal/engine"));

    // Link the Go static archive (macOS/Windows only)
    if (link_go) {
        const archive_path = if (go_archive.len > 0) b.path(go_archive) else b.path("internal/engine/go_engine.a");
        ...
        if (target.result.os.tag == .windows) {
            const target_file = if (go_archive.len > 0) go_archive else "internal/engine/go_engine.a";
